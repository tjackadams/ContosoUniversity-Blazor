...
namespace ContosoUniversity.IntegrationTests.Features.Students
{
    public class DetailsTests : IntegrationTestBase
    {
        ...
        public async Task Should_get_details()
        {
            ...
            {
                InstructorID = adminId, Budget = 123m, Name = "English 101", StartDate = DateTime.Today
            };
            ...
            {
                DepartmentID = deptId, Credits = 10, Id = NextCourseNumber(), Title = "Course 1"
            };
            var course2 = new Course
            {
                DepartmentID = deptId, Credits = 10, Id = NextCourseNumber(), Title = "Course 2"
            };
            await InsertAsync(course1, course2);

            var command = new Create.Command
            {
                FirstMidName = "Joe", LastName = "Schmoe", EnrollmentDate = new DateTime(2013, 1, 1)
            };
            var studentId = await SendAsync(command);

            var enrollment1 = new Enrollment { CourseID = course1.Id, Grade = Grade.A, StudentID = studentId };
            var enrollment2 = new Enrollment { CourseID = course2.Id, Grade = Grade.F, StudentID = studentId };
